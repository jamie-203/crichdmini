...
def extract_willow_stream():
    ...
    """
    # Step 1: Get the initial page to find the next script URL
    logging.info("--- Step 1: Fetching initial page ---")
    initial_content = get_page_content(INITIAL_URL)
    ...
    premium_js_content = get_page_content(premium_js_url, referrer=INITIAL_URL)
    ...
    # The 'fid' is needed for the iframe URL, get it from the first page's content
    ...
    player_page_content = get_page_content(iframe_url, referrer=INITIAL_URL)
    ...
    # Regex to find the array of characters in the treUptltHg function
    ...
    char_list = char_list_str.split(',')

    final_url = "".join([char.strip().strip('"').strip("'") for char in char_list])
